...
pub fn process_init_reserve(
    ...
    if !cfg!(feature = "anchor-test") {
        require_keys_eq!(
            *ctx.accounts.pyth_product.owner,
            ctx.accounts.lending_market.oracle_program_id,
            LendingError::InvalidOracleConfig
        );
        require_keys_eq!(
            *ctx.accounts.pyth_price.owner,
            ctx.accounts.lending_market.oracle_program_id,
            LendingError::InvalidOracleConfig
        );
    }
    ...
    require_keys_eq!(
        Pubkey::new_from_array(pyth_product.px_acc.to_bytes()),
        pyth_price_pubkey,
        LendingError::InvalidOracleConfig
    );
    ...
    // Actual deposit and mint are not done here, but we are updating values on DB (account variables) to sync.
    // Deposit and mint are done in the following two CPIs.
    let collateral_amount = ctx.accounts.reserve.deposit_liquidity(liquidity_amount)?;

    // Transfer user's liquidity to liquidity_supply held by lending_market
    transfer(
        ctx.accounts.into_transfer_user_liquidity_to_supply_ctx(),
        liquidity_amount,
    )?;

    // Mint collateral instead
    mint_to(
